...
def ligue_1_matchs(request):
    matchs_list = Events.objects.filter(organisateur__categorie='ligue 1')
    return render(request, "events_app/by_category.html", {'matchs_list': matchs_list})


def ligue_2_matchs(request):
    matchs_list = Events.objects.filter(organisateur__categorie='ligue 2')
    return render(request, "events_app/by_category.html", {'matchs_list': matchs_list})


def coupe_de_tunisie_matchs(request):
    matchs_list = Events.objects.filter(organisateur__categorie='Coupe du tunisie')
    return render(request, "events_app/by_category.html", {'matchs_list': matchs_list})
